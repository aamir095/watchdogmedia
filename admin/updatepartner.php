...
$flight = getPartnerById($conn,$flightId);
 ...
 if(isset($_POST['savebtn']))
 {
...

 	{
 		...
 		@unlink("uploads/".$flight['image_path']);
...
?>
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<select class="col-xs-10 col-sm-5" name="status">
								<option value="active" <?php if($flight['status']=='active') echo 'selected="selected"';  ?>>Active</option>
								<option value="in_active" <?php if($flight['status']=='in_active') echo 'selected="selected"';  ?>>In-Active</option>
							</select>
						</div>
					</div>

					<?php if(!empty($flight['image_path'])): ?>
					<div class="space-4"></div>
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Existing Image </label> 
						<div class="col-sm-9">
							<img src="uploads/<?php echo $flight['image_path'];?>" width=50%, height=200 px>
					</div>
				</div>
				<?php else: ?>
		...
		if(! ( /msie\s*(8|7|6)/.test(navigator.$flightAgent.toLowerCase())) ) 
		{
		tag_input.tag(
		{
